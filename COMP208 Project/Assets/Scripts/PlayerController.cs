...
public class PlayerController : MonoBehaviour {
    ...
    private void UpdateDirection() {
        float newDirection = input.actions["Move"].ReadValue<float>();
    ...
    public void OnDash(InputAction.CallbackContext context) {
        ...
        if (context.action.triggered && (Time.time - dashStartTime) > dashCooldown) {
            ...
            Vector2 newDirection = GetComponent<PlayerInput>().actions["Aim"].ReadValue<Vector2>();
    ...
    private void Teleport() {
        ...
        Vector2 destination = origin + (teleportDistance * teleportDirection);

        for (float distance = teleportDistance; distance > 0; distance -= step) {
            destination = origin + (teleportDirection * distance);
